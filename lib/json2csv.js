  ...
  params.data.forEach(function(data_element) {
    ...
    if (data_element && Object.getOwnPropertyNames(data_element).length > 0) {
      ...
      params.fields.forEach(function(field_element) {
        ...
        if (field_element.indexOf('.') !== -1) {
          var parts = field_element.split('.');
          var nested_element = data_element;
          for (var i = 0; i < parts.length; ++i) {
            if (nested_element.hasOwnProperty(parts[i])) {
              nested_element = nested_element[parts[i]];
            } else {
              nested_element = undefined;
              break;
            }
          }
          if (undefined !== nested_element) {
            line += JSON.stringify(nested_element);
          }
        } else if (data_element.hasOwnProperty(field_element)) {
