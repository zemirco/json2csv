...
module.exports = function (params, callback) {
  checkParams(params, function (err) {
    ...
    createColumnTitles(params, function (titlesError, title) {
      if (titlesError) {
        return callback(titlesError);
      }
  ...
  params.data.forEach(function (dataElement) {
    ...
    if (dataElement && Object.getOwnPropertyNames(dataElement).length > 0) {
      ...
      params.fields.forEach(function (fieldElement) {
        if (dataElement.hasOwnProperty(fieldElement)) {
          var stringifiedElement = JSON.stringify(dataElement[fieldElement]);
