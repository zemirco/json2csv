...
var checkParams = function(params, callback) {
  ...
  params.arrayDel = params.arrayDel || params.del;
  params.joinArray = params.joinArray || true;
...
var createColumnContent = function(params, str, callback) {
  params.data.forEach(function(data_element) {
    ...
    if (data_element && Object.getOwnPropertyNames(data_element).length > 0) {
      var line = '';
      var eol = os.EOL || '\n';
      var Arrayregex = new RegExp('"'+params.arrayDel+'"', 'g');
      params.fields.forEach(function(field_element) {
        if (data_element.hasOwnProperty(field_element)) {
          if (params.joinArray && typeOf(data_element[field_element]) == 'array') {
            for (var i = 0; i < data_element[field_element].length; i++) {
              data_element[field_element][i] = JSON.stringify(data_element[field_element][i]);
            };
            var currentLine = data_element[field_element].join(params.arrayDel);
            line += currentLine.replace(Arrayregex, params.arrayDel);
          } else {
