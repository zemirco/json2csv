...
function checkParams(params, callback) {
  ...
  if (params.defaultValues) {
    if (!Array.isArray(params.defaultValues)) {
      return callback(new Error('defaultValues must be an Array, if defaultValues is provided.'));
    }
    if (params.defaultValues.length !== params.fields.length) {
      return callback(new Error('defaultValues and fields should be of the same length, if defaultValues is provided.'));
    }
  } else {
    params.defaultValues = new Array(params.fields.length);
    params.fields.forEach(function (_, index) {
      params.defaultValues[index] = params.defaultValue;
    });
  }
...
function createColumnContent(params, str) {
  params.data.forEach(function (dataElement) {
    ...
    if (dataElement && Object.getOwnPropertyNames(dataElement).length > 0) {
      ...
      params.fields.forEach(function (fieldElement, index) {
        var val = lodashGet(dataElement, fieldElement, params.defaultValues[index]);
